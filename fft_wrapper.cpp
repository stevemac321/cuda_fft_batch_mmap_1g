...
{ 
  if (chunk_size >= CHUNK_THRESHOLD) {
    ...
    cuda_fft_pinned(mapdir, chunk_size);
  } else {
    cuda_fft_unpinned(mapdir, chunk_size);
  }
}
  void cuda_fft_pinned(const char *mapdir, const size_t chunk_size) {
#ifdef LOG_TELEMETRY
...
void cuda_fft_unpinned(const char *mapdir, const size_t chunk_size) 
{
#ifdef LOG_TELEMETRY
  struct SignalReport report;
#endif
